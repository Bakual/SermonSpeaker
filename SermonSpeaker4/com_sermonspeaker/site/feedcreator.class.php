...
$feedconfig =& JFactory::getConfig();
$tz = $feedconfig->getValue('config.offset');
if ($tz < 0) {$tzs = "-";} else {$tzs = "+";}
$atz = explode(".",$tz);
if ($atz[1] == "") {$atz[1] = "0";}
if (strlen(abs($atz[0])) == 1) {$btz = "0".abs($atz[0]);} else {$btz = abs($atz[0]);}
$tzs = $tzs.$btz.":".$atz[1]."0";
define("TIME_ZONE",$tzs);
...
class FeedItem extends HtmlDescribable {
	...
	var $itAuthor, $itBlock, $itCategory, $itDuration, $itExplicit, $itKeywords, $itSubtitle, $itSummary;
	...
	function output() {
		...
		if (!$this->rawFieldContent) {
			...
			if ($this->truncSize and is_int($this->truncSize)) {
				$result = FeedCreator::iTrunc(utfCleanText($this->rawFieldContent),$this->truncSize);
			} else {
				$result = utfCleanText($this->rawFieldContent);
			}
		}
		return $result;
	}
...
class FeedCreator extends HtmlDescribable {
	...
	// ADDED: iTunes specific variables
	// contact@jlleblanc.com
	
	var $itBlock, $itCategory, $itCategory1, $itCategory2, $itCategory3, $itExplicit, $itKeywords, $itImage, $itOwner, $itSubtitle, $itAuthor, $itSummary;
	...
	*/
	...
	//	Commented out for iTunes compatibility
...
class RSSCreator10 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= "        <title>".utfCleanText($this->title)."</title>\n";
		$feed.= "        <description>".utfCleanText($this->description)."</description>\n";
		$feed.= "        <link>".$this->link."</link>\n";
		if ($this->image!=null) {
			$feed.= "        <image rdf:resource=\"".$this->image->url."\" />\n";
		}
		$now = new FeedDate();
		$feed.= "       <dc:date>".utfCleanText($now->iso8601())."</dc:date>\n";
		$feed.= "        <items>\n";
		$feed.= "            <rdf:Seq>\n";
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "                <rdf:li rdf:resource=\"".utfCleanText($this->items[$i]->link)."\"/>\n";
		}
		$feed.= "            </rdf:Seq>\n";
		$feed.= "        </items>\n";
		$feed.= "    </channel>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "    <item rdf:about=\"".utfCleanText($this->items[$i]->link)."\">\n";
			//$feed.= "        <dc:type>Posting</dc:type>\n";
			$feed.= "        <dc:format>text/html</dc:format>\n";
			if ($this->items[$i]->date!=null) {
				$itemDate = new FeedDate($this->items[$i]->date);
				$feed.= "        <dc:date>".utfCleanText($itemDate->iso8601())."</dc:date>\n";
			}
			if ($this->items[$i]->source!="") {
				$feed.= "        <dc:source>".utfCleanText($this->items[$i]->source)."</dc:source>\n";
			}
			if ($this->items[$i]->author!="") {
				$feed.= "        <dc:creator>".utfCleanText($this->items[$i]->author)."</dc:creator>\n";
			}
			$feed.= "        <title>".utfCleanText(strip_tags(strtr($this->items[$i]->title,"\n\r","  ")))."</title>\n";
			$feed.= "        <link>".utfCleanText($this->items[$i]->link)."</link>\n";
			$feed.= "        <description>".utfCleanText($this->items[$i]->description)."</description>\n";
...
/*		$feed.= $this->_createGeneratorComment();
		...
		// Suggested improvement: change namespace url to all lowercase for validation purposes
		$feed .= "<rss version=\"2.0\" xmlns:itunes=\"http://www.itunes.com/dtds/podcast-1.0.dtd\">\n";

		$feed.= "    <channel>\n";
		$feed.= "        <title>".FeedCreator::iTrunc(utfCleanText($this->title),100)."</title>\n";

		$feed.= "        <link>".$this->link."</link>\n";
		$this->descriptionTruncSize = 500;
		$feed.= "        <description>".$this->getDescription()."</description>\n";

		if($this->itCategory1 != '' || $this->itCategory2 != '' || $this->itCategory3 != '') {
			$feed .= makeitCategory($this->itCategory1);
			$feed .= makeitCategory($this->itCategory2);
			$feed .= makeitCategory($this->itCategory3);
		}
		
		$now = new FeedDate();
		$feed.= "        <lastBuildDate>".utfCleanText($now->rfc822())."</lastBuildDate>\n";
		...
		if ($this->copyright!="") {
			$feed.= "        <copyright>".FeedCreator::iTrunc(utfCleanText($this->copyright),100)."</copyright>\n";
		}
		if ($this->editor!="") {
			$feed.= "        <managingEditor>".FeedCreator::iTrunc(utfCleanText($this->editor),100)."</managingEditor>\n";
		}
		if ($this->webmaster!="") {
			$feed.= "        <webMaster>".FeedCreator::iTrunc(utfCleanText($this->webmaster),100)."</webMaster>\n";
		}
		if ($this->pubDate!="") {
			$pubDate = new FeedDate($this->pubDate);
			$feed.= "        <pubDate>".$pubDate->rfc822()."</pubDate>\n";
		}
		if ($this->docs!="") {
			$feed.= "        <docs>".FeedCreator::iTrunc(utfCleanText($this->docs),500)."</docs>\n";
		}
		if ($this->ttl!="") {
			$feed.= "        <ttl>".utfCleanText($this->ttl)."</ttl>\n";
		}
		if ($this->rating!="") {
			$feed.= "        <rating>".FeedCreator::iTrunc(utfCleanText($this->rating),500)."</rating>\n";
		}
		if ($this->skipHours!="") {
			$feed.= "        <skipHours>".utfCleanText($this->skipHours)."</skipHours>\n";
		}
		if ($this->skipDays!="") {
			$feed.= "        <skipDays>".utfCleanText($this->skipDays)."</skipDays>\n";
		}

		if ($this->itSubtitle != "") {
			$feed .= "        <itunes:subtitle>" . utfCleanText($this->itSubtitle) . "</itunes:subtitle>\n";
		}
		if ($this->itAuthor != "") {
			$feed .= "        <itunes:author>" . utfCleanText($this->itAuthor) . "</itunes:author>\n";
		}
		if ($this->itSummary != "") {
			$feed .= "        <itunes:summary>" . utfCleanText($this->getDescription()) . "</itunes:summary>\n";
		}

		if (is_array($this->itOwner)) {
			$feed .= "        <itunes:owner>\n";

			if($this->itOwner['name'])
				$feed .= "                <itunes:name>" . utfCleanText($this->itOwner['name']) . "</itunes:name>\n";

			if($this->itOwner['email'])
				$feed .= "                <itunes:email>" . utfCleanText($this->itOwner['email']) . "</itunes:email>\n";
		...
		if ($this->itImage != "") {
			$feed .= "        <itunes:image href=\"" . utfCleanText($this->itImage) . "\" />";
			$feed .= "        <image>\n";
			$feed .= "                <link>" . $this->link . "</link>\n";
			$feed .= "                <url>" . utfCleanText($this->itImage) . "</url>\n";
			$feed .= "                <title>" . FeedCreator::iTrunc(utfCleanText($this->title), 100) . "</title>\n";
			$feed .= "                <description>\n".$this->getDescription()."\n</description>\n";
			$feed .= "        </image>\n";
		}
						
		// iTunes specific tags: Applicable to entire feed
		// contact@jlleblanc.com
		...
		/* changed by MHE for Sermonspeaker
		if ($this->itExplicit == 1) {
		...
		*/		
		...
		if ($this->itKeywords != "") {
			$feed .= "        <itunes:keywords>" . utfCleanText($this->itKeywords) . "</itunes:keywords>\n";
		}
    
    //next 3 lines added by MHE for SermonSpeaker to support the new-feed-url
	  if ($this->newfeedurl) {
      $feed.= "        <itunes:new-feed-url>".utfCleanText($this->newfeedurl)."</itunes:new-feed-url>\n";
    }
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			$feed.= "            <title>".FeedCreator::iTrunc(utfCleanText(strip_tags($this->items[$i]->title)),100)."</title>\n";
			$feed.= "            <link>".utfCleanText($this->items[$i]->link)."</link>\n";
      
      //next 3 lines removed by MHE for SermonSpeaker
      /*
			if ($this->itAuthor) {
				$feed.= "            <author>test".utfCleanText($this->itAuthor)."</author>\n";
			}
			*/
			
			//next line added by MHE for SermonSpeaker
      $feed.= "            <author>".$this->items[$i]->author."</author>\n";
			
      $feed.= "            <description>".$this->items[$i]->getDescription()."</description>\n";

			if ($this->items[$i]->category!="") {
				$feed.= "            <category>".utfCleanText($this->items[$i]->category)."</category>\n";
			}
			if ($this->items[$i]->comments!="") {
				$feed.= "            <comments>".utfCleanText($this->items[$i]->comments)."</comments>\n";
			}
			if ($this->items[$i]->date!="") {
			$itemDate = new FeedDate($this->items[$i]->date);
				$feed.= "            <pubDate>".utfCleanText($itemDate->rfc822())."</pubDate>\n";
			}
			if ($this->items[$i]->guid!="") {
				$feed.= "            <guid>".utfCleanText($this->items[$i]->guid)."</guid>\n";
			}

			
			// iTunes specific: per item
			if ($this->items[$i]->itAuthor != "") {
				$feed .= "            <itunes:author>" . utfCleanText($this->items[$i]->itAuthor) . "</itunes:author>\n";
			}

			if ($this->items[$i]->itBlock == 1) {
				$feed .= "            <itunes:block />\n";
			}
			if ($this->items[$i]->itDuration != "") {
				$feed .= "            <itunes:duration>" . utfCleanText($this->items[$i]->itDuration) . "</itunes:duration>\n";
			}
			/* changed by MHE for Sermonspeaker
			if ($this->items[$i]->itExplicit == 1) {
				$feed .= "            <itunes:explicit />\n";
			}
			*/
			if ($this->items[$i]->itExplicit == 1) {
			...
			if ($this->items[$i]->itKeywords != "") {
				$feed .= "            <itunes:keywords>" . utfCleanText($this->items[$i]->itKeywords) . "</itunes:keywords>\n";
			}
			if ($this->items[$i]->itSubtitle != "") {
				$feed .= "            <itunes:subtitle>" . utfCleanText($this->items[$i]->itSubtitle) . "</itunes:subtitle>\n";
			}
			if ($this->items[$i]->itSummary != "") {
				$feed .= "            <itunes:summary>" . utfCleanText($this->items[$i]->itSummary) . "</itunes:summary>\n";
			}
...
class PIECreator01 extends FeedCreator {
	...
	function createFeed() {
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			$feed.= "        <link>".utfCleanText($this->items[$i]->link)."</link>\n";
			$itemDate = new FeedDate($this->items[$i]->date);
			$feed.= "        <created>".utfCleanText($itemDate->iso8601())."</created>\n";
			$feed.= "        <issued>".utfCleanText($itemDate->iso8601())."</issued>\n";
			$feed.= "        <modified>".utfCleanText($itemDate->iso8601())."</modified>\n";
			$feed.= "        <id>".utfCleanText($this->items[$i]->guid)."</id>\n";
			if ($this->items[$i]->author!="") {
				$feed.= "        <author>\n";
				$feed.= "            <name>".utfCleanText($this->items[$i]->author)."</name>\n";
...
class AtomCreator03 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= ">\n"; 
		$feed.= "    <title>".utfCleanText($this->title)."</title>\n";
		$feed.= "    <tagline>".utfCleanText($this->description)."</tagline>\n";
		$feed.= "    <link rel=\"alternate\" type=\"text/html\" href=\"".utfCleanText($this->link)."\"/>\n";
		$feed.= "    <id>".utfCleanText($this->link)."</id>\n";
		$now = new FeedDate();
		$feed.= "    <modified>".utfCleanText($now->iso8601())."</modified>\n";
		if ($this->editor!="") {
			$feed.= "    <author>\n";
		...
		for ($i=0;$i<count($this->items);$i++) {
			$feed.= "    <entry>\n";
			$feed.= "        <title>".utfCleanText(strip_tags($this->items[$i]->title))."</title>\n";
			$feed.= "        <link rel=\"alternate\" type=\"text/html\" href=\"".utfCleanText($this->items[$i]->link)."\"/>\n";
			if ($this->items[$i]->date=="") {
			...
			$feed.= "        <issued>".utfCleanText($itemDate->iso8601())."</issued>\n";
			$feed.= "        <modified>".utfCleanText($itemDate->iso8601())."</modified>\n";
			$feed.= "        <id>".utfCleanText($this->items[$i]->link)."</id>\n";
			$feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
			if ($this->items[$i]->author!="") {
				$feed.= "        <author>\n";
				$feed.= "            <name>".utfCleanText($this->items[$i]->author)."</name>\n";
			...
			if ($this->items[$i]->description!="") {
				$feed.= "        <summary>".utfCleanText($this->items[$i]->description)."</summary>\n";
			}
			$feed.= "    </entry>\n";
		}
		$feed.= "</feed>\n";
...
class OPMLCreator extends FeedCreator {
	...
	function createFeed() {     
		...
		for ($i=0;$i<count($this->items);$i++) {
			...
			$title = utfCleanText(strip_tags(strtr($this->items[$i]->title,"\n\r","  ")));
			$feed.= " title=\"".$title."\"";
			$feed.= " text=\"".$title."\"";
			//$feed.= " description=\"".utfCleanText($this->items[$i]->description)."\"";
			$feed.= " url=\"".utfCleanText($this->items[$i]->link)."\"";
			$feed.= "/>\n";
		}
		$feed.= "    </body>\n";
		$feed.= "</opml>\n";
	...
	function createFeed() {
		...
		if ($this->image!=null) {
			$imageStr = "<a href='".$this->image->link."'".$targetInsert.">".
							...
							FeedCreator::iTrunc(utfCleanText($this->image->title),100).
		...
		if ($this->title) {
			$feedArray[] = "<div class='".$this->stylePrefix."title'><a href='".$this->link."' ".$targetInsert." class='".$this->stylePrefix."title'>".
				FeedCreator::iTrunc(utfCleanText($this->title),100)."</a></div>";
		}
			...
			if ($this->items[$i]->title) {
				if ($this->items[$i]->link) {
					$feedArray[] = 
						...
						"item_title'".$targetInsert.">".FeedCreator::iTrunc(utfCleanText(strip_tags($this->items[$i]->title)),100).
						"</a></div>";
				} else {
					$feedArray[] = 
						"<div class='".$this->stylePrefix."item_title'>".
						FeedCreator::iTrunc(utfCleanText(strip_tags($this->items[$i]->title)),100).
						"</div>";
				}
			}
...
function makeitCategory($cat)
{
	...
	if($child == '')
	{
		$tags = '        <itunes:category text="'. utfCleanText($parent) .'" />' . "\n";
	}
	else
	{
		$tags = '        <itunes:category text="'. utfCleanText($parent) .'">' . "\n";
		$tags .= '               <itunes:category text="'. utfCleanText($child) .'" />' . "\n";
