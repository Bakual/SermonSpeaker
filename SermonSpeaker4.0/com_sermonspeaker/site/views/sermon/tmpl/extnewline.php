...
<?php if ($this->params->get('hide_dl') == "0" && strlen($this->row->sermon_path) > 0) : ?>
	<h3 class="contentheading"><a title="<?php echo JText::_('COM_SERMONSPEAKER_DIRECTLINK_HOOVER'); ?>" href="<?php echo $this->lnk; ?>"><?php echo $this->escape($this->row->sermon_title); ?></a></h3>
<?php else : ?>
...
<div class="ss-sermondetail-container">
	...
	<?php if (in_array('sermon:scripture', $columns) && $this->row->sermon_scripture) : ?>
	...
	<?php endif;
	if (in_array('sermon:custom1', $columns) && strlen($this->row->custom1) > 0) : ?>
	...
	<?php endif;
	if (in_array('sermon:custom2', $columns) && strlen($this->row->custom2) > 0) : ?>
	...
	<?php if (in_array('sermon:notes', $columns) && strlen($this->row->notes) > 0) : ?>
