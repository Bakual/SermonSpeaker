...
class SermonspeakerModelFeed extends JModelLegacy
{
	function getData()
	{
		...
		$query->select('sermons.sermon_date, sermons.title, sermons.audiofile, sermons.videofile, sermons.notes, sermons.sermon_time, sermons.id, sermons.picture');
		...
		// Join over Speaker
		$query->select('speakers.title AS speakers_title, speakers.pic AS pic');
