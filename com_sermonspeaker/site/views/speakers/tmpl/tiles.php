	...
	<?php else : ?>
		<?php foreach ($this->items as $i => $item) :
			...
			if (in_array('speakers:intro', $this->col_speaker) and $item->intro) :
				$tip[] = Text::_('COM_SERMONSPEAKER_FIELD_INTRO_LABEL') . ': ' . JHtml::_('content.prepare', $item->intro);
			endif;

			if (in_array('speakers:bio', $this->col_speaker) and $item->bio) :
				$tip[] = Text::_('COM_SERMONSPEAKER_FIELD_BIO_LABEL') . ': ' . JHtml::_('content.prepare', $item->bio);
			endif;
