<?php
defined('_JEXEC') or die;

jimport('joomla.application.component.controllerform');

/**
 * Serie controller class.
 *
 * @package		SermonSpeaker.Administrator
 */
class SermonspeakerControllerSerie extends JControllerForm
{
	/**
	 * Method override to check if you can add a new record.
	 *
	 * @param	array	$data	An array of input data.
	 * @return	boolean
	 */
	protected function allowAdd($data = array())
	{
		$user		= JFactory::getUser();
		$categoryId	= JArrayHelper::getValue($data, 'catid', JFactory::getApplication()->input->get('filter_category_id'), 'int');
		$allow = null;

		if ($categoryId)
		{
			// If the category has been passed in the data or URL check it.
			$allow = $user->authorise('core.create', $this->option.'.category.'.$categoryId);
		}

		if ($allow === null)
		{
			// In the absense of better information, revert to the component permissions.
			return parent::allowAdd($data);
		}
		else
		{
			return $allow;
		}
	}

	/**
	 * Method to check if you can add a new record.
	 *
	 * @param	array	$data	An array of input data.
	 * @param	string	$key	The name of the key for the primary key.
	 *
	 * @return	boolean
	 */
	protected function allowEdit($data = array(), $key = 'id')
	{
		$recordId = (int) isset($data[$key]) ? $data[$key] : 0;
		$categoryId = 0;

		if ($recordId)
		{
			// Need to do a lookup from the model.
			$record = $this->getModel()->getItem($recordId);
			$categoryId = (int) $record->catid;
		}

		if ($categoryId)
		{
			$user = JFactory::getUser();
			// The category has been set. Check the category permissions.
			if ($user->authorise('core.edit', $this->option.'.category.'.$categoryId))
			{
				return true;
			}
			// Fallback on edit.own.
			if ($user->authorise('core.edit.own', $this->option.'.category.'.$categoryId))
			{
				return ($record->created_by == $user->get('id'));
			}
		}
		else
		{
			// Since there is no asset tracking, revert to the component permissions.
			return parent::allowEdit($data, $key);
		}
	}

	public function reset()
	{
		$app	= JFactory::getApplication();
		$jinput	= $app->input;
		$db		= JFactory::getDBO();
		$id 	= $jinput->get('id', 0, 'int');
		if (!$id)
		{
			$app->redirect('index.php?option=com_sermonspeaker&view=series', JText::_('JERROR_AN_ERROR_HAS_OCCURRED'), 'error');
			return;
		}
		$model 	= $this->getModel();
		$item 	= $model->getItem($id);
		$user	= JFactory::getUser();
		$canEdit	= $user->authorise('core.edit', 'com_sermonspeaker.category.'.$item->catid);
		$canEditOwn	= $user->authorise('core.edit.own', 'com_sermonspeaker.category.'.$item->catid) && $item->created_by == $user->id;
		if ($canEdit || $canEditOwn)
		{
			$query	= "UPDATE #__sermon_series \n"
					. "SET hits='0' \n"
					. "WHERE id='".$id."'"
					;
			$db->setQuery($query);
			$db->execute();
			$app->redirect('index.php?option=com_sermonspeaker&view=series', JText::sprintf('COM_SERMONSPEAKER_RESET_OK', JText::_('COM_SERMONSPEAKER_SERIE'), $item->title));
		}
		else
		{
			$app->redirect('index.php?option=com_sermonspeaker&view=series', JText::_('JERROR_ALERTNOAUTHOR'), 'error');
		}
		return;
	}

	/**
	 * Method to run batch operations.
	 *
	 * @param   object  $model  The model.
	 *
	 * @return  boolean	 True if successful, false otherwise and internal error is set.
	 *
	 * @since   1.7
	 */
	public function batch($model = null)
	{
		JSession::checkToken() or jexit(JText::_('JINVALID_TOKEN'));

		// Set the model
		$model = $this->getModel('Serie', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_sermonspeaker&view=series'.$this->getRedirectToListAppend(), false));

		if (!$this->input->post->get('newBatchProcess', 0, 'int'))
		{
			// Old way
			return parent::batch($model);
		}
		else
		{
			$vars = $this->input->post->get('batch', array(), 'array');
			$cid  = $this->input->post->get('cid', array(), 'array');
			$cid  = array_unique($cid);
			JArrayHelper::toInteger($cid);

			// Remove any values of zero.
			if (array_search(0, $cid, true))
			{
				unset($cid[array_search(0, $cid, true)]);
			}

			if (empty($cid))
			{
				$this->setMessage(JText::sprintf('JLIB_APPLICATION_ERROR_BATCH_FAILED', JText::_('JGLOBAL_NO_ITEM_SELECTED')));
				return false;
			}

			// Init some variables
			$contexts = array();
			$done = false;

			foreach ($cid as $id)
			{
				// If we're coming from com_categories, we need to use extension vs. option
				$option  = isset($this->extension) ? $this->extension : $this->option;
				$context = $option . '.' . $this->context . '.' . $id;

				$item = $model->getItem($id);

				// Category
				if (!empty($commands['category_id']))
				{
					$cmd = JArrayHelper::getValue($commands, 'move_copy', 'c');

					if ($cmd == 'c')
					{
						$item = $model->batchCategoryCopy($item, $commands['category_id'], $context);
						if ($model->getError())
						{
							break;
						}
					}
					else
					{
						$item = $model->batchCategoryMove($item, $commands['category_id'], $context
						if ($model->getError())
						{
							break;
						}
					}
					$done = true;
				}

				// Assetgroup
				if (!empty($commands['assetgroup_id']))
				{
					$item = $model->batchAssetgroup($item, $commands['assetgroup_id'], $context);
					if ($model->getError())
					{
						break;
					}

					$done = true;
				}

				// Language
				if (!empty($commands['language_id']))
				{
					$item = $model->batchLanguage($item, $commands['language_id'], $context);
					// Existing function, needs a check then is_array($id) and threat it as $item.
					if ($model->getError())
					{
						break;
					}

					$done = true;
				}

				// Tag
				if (!empty($commands['tag']))
				{
					$item = $model->batchTag($item, $commands['tag'], $context);
					// Existing function, needs a check then is_array($id) and threat it as $item.
					if ($model->getError())
					{
						break;
					}

					$done = true;
				}

				// Nothing to do
				if (!$done)
				{
					$model->setError(JText::_('JLIB_APPLICATION_ERROR_INSUFFICIENT_BATCH_INFORMATION'));
					break;
				}
			}

			if ($model->getError())
			{
				$this->setMessage(JText::sprintf('JLIB_APPLICATION_ERROR_BATCH_FAILED', $model->getError()));
				return false;
			}
			$this->setMessage(JText::_('JLIB_APPLICATION_SUCCESS_BATCH'));

			// Clear the cache
			$model->cleanCache();

			return true;
		}
	}

	protected function getRedirectToItemAppend($recordId = null, $urlVar = null)
	{
		$append = parent::getRedirectToItemAppend($recordId, $urlVar);
		$modal	= JFactory::getApplication()->input->get('modal', 0, 'int');
		$return	= $this->getReturnPage();

		if ($modal) {
			$append .= '&tmpl=component';
		}

		if ($return) {
			$append .= '&return='.base64_encode($return);
		}

		return $append;
	}

	/**
	 * Get the return URL.
	 *
	 * If a "return" variable has been passed in the request
	 *
	 * @return	string	The return URL.
	 * @since	1.6
	 */
	protected function getReturnPage()
	{
		$return = JFactory::getApplication()->input->get('return', '', 'base64');

		if (empty($return) || !JUri::isInternal(base64_decode($return))) {
			return false;
		}
		else {
			return base64_decode($return);
		}
	}

	/**
	 * Method to save a record.
	 *
	 * @param	string	$key	The name of the primary key of the URL variable.
	 * @param	string	$urlVar	The name of the URL variable if different from the primary key (sometimes required to avoid router collisions).
	 *
	 * @return	Boolean	True if successful, false otherwise.
	 * @since	1.6
	 */
	public function save($key = null, $urlVar = 'id')
	{
		$result = parent::save($key, $urlVar);

		// If ok, redirect to the return page.
		if ($result && ($return = $this->getReturnPage())) {
			$this->setRedirect($return);
		}

		return $result;
	}
}